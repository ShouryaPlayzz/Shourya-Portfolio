...
(function () {
  ...
  // ==========================================================================
  // 13. CONTACT FORM - WEB3FORMS INTEGRATION & SUBMISSION LOG
  // ==========================================================================
  const WEB3FORMS_ACCESS_KEY = 'ad6a84ee-2f60-4603-a55b-ba00788d6095';
  ...
  function updateInquiriesBadge() {
    try {
      ...
      const modalCount = document.getElementById('modal-inquiries-count');
  ...
  function initContactForm() {
    ...
    function updateCharCounter() {
      ...
      charCounter.textContent = `${len} / 600 chars`;
      if (len >= 550) {
        charCounter.classList.add('text-amber-500', 'font-bold');
        charCounter.classList.remove('text-slate-400', 'dark:text-slate-500');
      } else {
    ...
    // Audio & tactile feedback for service and budget chips
    const optionInputs = form.querySelectorAll('input[name="service"], input[name="budget"]');
    ...
    form.addEventListener('submit', async (e) => {
      ...
      [nameInput, emailInput].forEach(input => {
        if (input) {
          ...
          const emailValid = !isEmail || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val);
          if (!val || !emailValid) {
      ...
      if (hasError) {
        sound.playClick();
        showToast('Please provide your name and a valid email address.', 'error');
        return;
      }
      ...
      submitBtn.innerHTML = `<span class="material-symbols-outlined text-base animate-spin">progress_activity</span> Transmitting to Shourya...`;

      // Save inquiry to localStorage archive
      const inquiry = {
        name,
        email,
        message,
        services: services.length ? services : ['General Inquiry'],
        budget,
      ...
      try {
        const stored = JSON.parse(localStorage.getItem('sr_inquiries') || '[]');
        stored.unshift(inquiry); // newest first
      ...
      try {
        ...
        const response = await fetch('https://api.web3forms.com/submit', {
          ...
          body: JSON.stringify({
            ...
            budget: budget,
            message: message,
            from_name: 'Shourya Rana Portfolio',
            subject: `✨ New Commission Brief from ${name} (Portfolio Inquiry)`
          })
        });
        ...
        if (response.status === 200 || result.success) {
          ...
          submitBtn.innerHTML = `<span class="material-symbols-outlined text-base text-emerald-400">check_circle</span> Dispatched to Shourya's Inbox!`;
          ...
          showToast(`Brief delivered straight to Shourya's email!`, 'success');
          form.reset();
          updateCharCounter();
        } else {
        ...
        console.warn('Web3Forms dispatch error or offline:', error);
        // Graceful fallback - already saved locally!
        submitBtn.disabled = false;
        submitBtn.innerHTML = `<span class="material-symbols-outlined text-base text-emerald-400">check_circle</span> Brief Saved &amp; Queued!`;

        triggerConfetti();
        sound.playSuccess();
        showToast(`Brief backed up locally! Shourya will reply within 4 hours.`, 'success');
        form.reset();
        updateCharCounter();
      } finally {
        setTimeout(() => {
  ...
  // ==========================================================================
  // 14. CLIENT INQUIRIES LOG MODAL (ADMIN / SUBMISSIONS VIEWER)
  // ==========================================================================
  function initInquiriesModal() {
    ...
    const closeBtn = document.getElementById('inquiries-close-btn');
    const exportBtn = document.getElementById('inquiries-export-btn');
    const clearBtn = document.getElementById('inquiries-clear-btn');
    ...
    function renderList() {
      ...
      let stored = [];
      try {
        stored = JSON.parse(localStorage.getItem('sr_inquiries') || '[]');
      } catch (e) {}
      ...
      if (stored.length === 0) {
        container.innerHTML = `
          <div class="py-12 px-4 flex flex-col items-center justify-center text-center gap-2">
            <div class="p-3 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-400">
              <span class="material-symbols-outlined text-3xl">inbox</span>
            </div>
            <h4 class="font-bold text-base text-slate-800 dark:text-slate-200">No Submissions Recorded Yet</h4>
            <p class="text-xs text-slate-500 dark:text-slate-400 max-w-sm">
              When prospective clients fill out the contact form, submissions will appear here and be dispatched straight to your inbox via Web3Forms.
            </p>
          </div>
        `;
        return;
      ...
      container.innerHTML = stored
        .map((inq, idx) => {
          ...
          const servicesList = Array.isArray(inq.services) ? inq.services.join(', ') : inq.services;

          return `
            <div class="p-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 shadow-sm flex flex-col gap-3">
              <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-100 dark:border-slate-700">
                <div class="flex items-center gap-2.5">
                  <div class="h-9 w-9 rounded-xl bg-blue-100 dark:bg-blue-900/60 text-primary dark:text-blue-300 flex items-center justify-center font-bold text-sm">
                    ${inq.name ? inq.name.charAt(0).toUpperCase() : 'C'}
                  </div>
                  <div class="flex flex-col">
                    <span class="font-bold text-sm text-slate-900 dark:text-white">${inq.name || 'Anonymous Client'}</span>
                    <a href="mailto:${inq.email}" class="text-xs text-primary dark:text-blue-400 hover:underline font-mono-code">${inq.email || 'No email provided'}</a>
                  </div>
                </div>

                <div class="flex items-center gap-2 text-xs font-mono-code text-slate-500 dark:text-slate-400">
                  <span class="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 font-medium">${timeStr}</span>
                  <button class="copy-inquiry-btn p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500 hover:text-primary transition-colors" data-index="${idx}" title="Copy brief details">
              ...
              <div class="flex flex-wrap items-center gap-2 text-xs">
                <span class="font-label-caps text-[10px] font-bold text-slate-400 uppercase">SERVICES:</span>
                <span class="px-2 py-0.5 rounded-md bg-blue-50 dark:bg-blue-950/60 text-primary dark:text-blue-300 font-mono-code font-medium">${servicesList}</span>
                <span class="font-label-caps text-[10px] font-bold text-slate-400 uppercase ml-2">BUDGET:</span>
                <span class="px-2 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 font-mono-code font-medium">${inq.budget || 'Unspecified'}</span>
              </div>

              <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/70 border border-slate-100 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 leading-relaxed font-mono-code whitespace-pre-wrap">
                ${inq.message || 'No project description attached.'}
              </div>
            </div>
          `;
        })
      ...
      container.querySelectorAll('.copy-inquiry-btn').forEach((btn) => {
        btn.addEventListener('click', () => {
          const index = parseInt(btn.dataset.index, 10);
          const inq = stored[index];
          if (!inq) return;
          const text = `CLIENT INQUIRY:\nName: ${inq.name}\nEmail: ${inq.email}\nServices: ${Array.isArray(inq.services) ? inq.services.join(', ') : inq.services}\nBudget: ${inq.budget}\nTime: ${inq.timestamp}\n\nProject Scope:\n${inq.message}`;
          navigator.clipboard.writeText(text).then(() => {
            sound.playSuccess();
            showToast('Inquiry brief copied to clipboard!', 'success');
          });
        });
      });
    }
    ...
    if (exportBtn) {
      exportBtn.addEventListener('click', () => {
        ...
        const headers = ['Timestamp', 'Name', 'Email', 'Services', 'Budget', 'Message'];
        const rows = stored.map((inq) => [
          `"${inq.timestamp || ''}"`,
          `"${(inq.name || '').replace(/"/g, '""')}"`,
          `"${(inq.email || '').replace(/"/g, '""')}"`,
          `"${(Array.isArray(inq.services) ? inq.services.join('; ') : inq.services || '').replace(/"/g, '""')}"`,
        ...
        a.href = url;
        a.download = `shourya_inquiries_${new Date().toISOString().slice(0, 10)}.csv`;
        a.click();
        URL.revokeObjectURL(url);
        sound.playSuccess();
        showToast('Inquiries exported as CSV', 'success');
      });
    }
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', () => {
        if (confirm('Are you sure you want to clear all locally archived inquiries?')) {
          localStorage.removeItem('sr_inquiries');
          renderList();
          updateInquiriesBadge();
          sound.playClick();
          showToast('Inquiry log cleared', 'info');
        }
      });
    }
  }
  ...
  // ==========================================================================
  // 15. THEME TOGGLE & SOUND TOGGLE CONTROLLERS
  // ==========================================================================
  ...
  // ==========================================================================
  // 16. SCROLL SPY & ACTIVE NAVBAR HIGHLIGHTING
  // ==========================================================================
  function initScrollSpy() {
    if (window.location.pathname.includes('hire.html') || window.location.href.includes('hire.html')) return;

    const nav = document.getElementById('main-nav');
    if (!nav) return;
    const navLinks = nav.querySelectorAll('.nav-pill');
    if (!navLinks.length) return;

    const sectionIds = ['hire-cta', 'dev-section', 'design-section', 'video-section', 'showcases', 'journey', 'top'];
    ...
    function updateActive(activeId) {
      navLinks.forEach(link => {
        ...
        const isMatch = target === activeId || (activeId === 'hire-cta' && href === 'hire.html');
        if (isMatch) {
